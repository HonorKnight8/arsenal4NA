...
class PreferencesPersonal extends Preferences
{
    protected function thisPage()    //本页面特有信息
    {
        ...
        $this->div_preferences .= '&emsp;&emsp;&emsp;<input type="file" name="file" id="file" placeholder="选择要上传的照片" accept="image/jpeg, image/gif, image/png, image/webp, image/bmp" /><br />';
        ...
        $this->div_preferences .= '&emsp;&emsp;&emsp;<input type="radio" name="showHeadPhoto" value="1"';
        if ($staffInfoArray['showHeadPhoto'] == 1) {
            $this->div_preferences .= 'checked';
        }
        $this->div_preferences .= '>显示照片';
        $this->div_preferences .= '&emsp;&emsp;&emsp;<input type="radio" name="showHeadPhoto" value="0"';
        if ($staffInfoArray['showHeadPhoto'] == 0) {
            $this->div_preferences .= 'checked';
        }
        $this->div_preferences .= '>隐藏照片<br />';
        $this->div_preferences .= '注意：显示/隐藏照片选项，对全员生效<br />';
        $this->div_preferences .= '&emsp;&emsp;&emsp;<input type="submit" name="PreferencesPersonal_1" value="上传" />';
