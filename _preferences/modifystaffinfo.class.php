<?php
class ModifyStaffInfo extends Preferences
{
    protected function thisPage()    //本页面特有信息
    {
        //读取get传值的StaffID
        $targetStaffID = isset($_REQUEST["StaffID"]) ? $_REQUEST["StaffID"] : $_SESSION['staffID'];
        // echo $targetStaffID;

        $staffInfoArray = Contacts::getStaffInfo($targetStaffID, 1);

        $titlle = '<br /><span><b>这是管理员新建/修改员工信息修改页</b></span><br />';

        $form1 = PreferencesPersonal::changeHeadPhotoDiv($staffInfoArray, $_SESSION['staffID'], $targetStaffID);
        ...
        $form2 .= '<img src="" alt="" id="verifyimage_2" /><a class="a_in_content" onclick="document.getElementById(\'verifyimage_2\').src=\'_libs/captchaget0.php?r=\'+Math.random()" href="javascript:void(0)">显示/更换验证码</a><br />';
        $form2 .= '&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;<input class="submit" type="submit" name="PreferencesPersonal_2" value="修改密码" />';
        $form2 .= '</fieldset></form></div>';

        //修改其他信息的部分
        $form3 = PreferencesPersonal::modifyStaffInfoDiv($staffInfoArray, $_SESSION['staffID'], $staffInfoArray);
    ...
    // /**
    //  * 输出修改头像表单
    //  * @param   array   $staffInfoArray     当前员工信息数组
    //  * @param   string  $executorID         操作者ID；
    //  * @param   string  $staffID            操作目标ID；
    //  * @return  string  $changeHeadPhotoDiv 返回表单
    //  */
    // static function changeHeadPhotoDiv($staffInfoArray, $executorID, $staffID)  //OK
    // {
    //     $changeHeadPhotoDiv = '<div>';
    //     $changeHeadPhotoDiv .= '<form action="" method="post" enctype="multipart/form-data">';
    //     $changeHeadPhotoDiv .= '<fieldset><legend>头像照片设置</legend>';
    //     $changeHeadPhotoDiv .= '<img src="' . $staffInfoArray['headPhoto'] . '" style="float:right;" />';
    //     $changeHeadPhotoDiv .= '&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;<input type="file" name="file" id="file" placeholder="选择要上传的照片" accept="image/jpeg, image/gif, image/png, image/webp, image/bmp" /><br />';
    //     $changeHeadPhotoDiv .= '&emsp;&emsp;&emsp;注意：支持的文件格式有：jpg、gif、png、webp、bmp<br />';
    //     $changeHeadPhotoDiv .= '&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;建议照片的横竖比例等于或接近于：1:1.45(证件照比例)<br />';
    //     $changeHeadPhotoDiv .= '&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;照片大小不得大于1MB<br />';
    //     $changeHeadPhotoDiv .= '&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;<input type="radio" name="showHeadPhoto" value="1" ';
    //     $changeHeadPhotoDiv .= $staffInfoArray['showHeadPhoto'] == 1 ? 'checked' : '';
    //     $changeHeadPhotoDiv .= $executorID !== $staffID ? ' disabled' : ''; //只有自己才能设置是否显示照片
    //     $changeHeadPhotoDiv .= ' />显示照片';
    //     $changeHeadPhotoDiv .= '&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;<input type="radio" name="showHeadPhoto" value="0" ';
    //     $changeHeadPhotoDiv .= $staffInfoArray['showHeadPhoto'] == 0 ? 'checked' : '';
    //     $changeHeadPhotoDiv .= $executorID !== $staffID ? ' disabled' : '';
    //     $changeHeadPhotoDiv .= ' />隐藏照片<br />';
    //     $changeHeadPhotoDiv .= '&emsp;&emsp;&emsp;注意：显示/隐藏照片选项，对全员生效<br />';
    //     $changeHeadPhotoDiv .= '输入验证码：<input type="text" name="inputcaptchaget_1" id="" placeholder="请输入右侧的验证码……" required autocomplete="off">';
    //     $changeHeadPhotoDiv .= '<img src="" alt="" id="verifyimage_1" /><a class="a_in_content" onclick="document.getElementById(\'verifyimage_1\').src=\'_libs/captchaget0.php?r=\'+Math.random()" href="javascript:void(0)">显示/更换验证码</a><br />';
    //     $changeHeadPhotoDiv .= '&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;<input class="submit" type="submit" name="PreferencesPersonal_1" value="上传" />';
    //     $changeHeadPhotoDiv .= '</fieldset></form></div>';

    //     return $changeHeadPhotoDiv;
    // }
    ...
    //  * @param   string  $executorID         操作者ID；
    //  * @param   string  $staffID            操作目标ID；
    //  * @return  string  $modifyStaffInfoDiv 返回表单
    //  */
    ...
    //     $modifyStaffInfoDiv .= '<img src="" alt="" id="verifyimage_3" /><a class="a_in_content" onclick="document.getElementById(\'verifyimage_3\').src=\'_libs/captchaget0.php?r=\'+Math.random()" href="javascript:void(0)">显示/更换验证码</a><br />';
    //     $modifyStaffInfoDiv .= '&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;<input class="submit" type="submit" name="PreferencesPersonal_3" value="修改" />';
